	...
	<div class="flex w-full grow flex-col items-center justify-center gap-4">
		...
		<div class="flex flex-wrap items-center gap-4">
			<div class="input-group-bordered w-min">
				<input v-model="searchItem" type="text" class="input !input-sm border-2 !border-secondary" placeholder="Search a Pokémon...">
		...
		<div class="flex grow flex-wrap items-center justify-center gap-4">
				...
				<div class="flex max-w-fit grow select-none flex-wrap justify-items-center rounded-xl bg-base-200 sm:p-2">
					<template v-if="searchFilter.length">
						<div v-for="pok in searchFilter" :key="pok.index"
							...
							<img loading="lazy" class="mb-1 h-12 w-12 transition-all sm:h-16 sm:w-16" :src="`/sprites/gen9/${parseInt(pok.ndex)}.png`">
			...
			<Disclosure v-for="(gen, i) in pokGens" v-else :key="`gen_${i+1}`" v-slot="{ open }" :defaultOpen="true" as="div" 
				class="max-w-fit grow rounded-xl bg-base-200 p-2">
				...
				<Transition
					...
					<DisclosurePanel as="div" class="flex select-none flex-wrap items-center justify-center gap-y-4 text-2xs sm:p-2 sm:text-xs">
		...
		<button class="btn btn-circle btn-secondary sticky bottom-6 z-50 self-end" 
