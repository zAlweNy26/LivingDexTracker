...
<template>
	<section class="flex w-full grow flex-col gap-4">
		...
		<div class="flex flex-wrap self-center items-center gap-4">
			<NuSelect v-model="spriteType" highlight defaultValue="1" :items="[
				{ label: $t('pokemon.icon'), value: '1' },
				{ label: $t('pokemon.model'), value: '2' },
				{ label: $t('pokemon.shiny'), value: '3' },
			]" />
			<NuSwitch v-model="showChineseExclusives" :label="$t('quest.chineseExclusives')" />
			<NuSwitch v-if="showChineseExclusives && !onlySprites" v-model="showChineseNames" :label="$t('quest.chineseNames')" />
		</div>
