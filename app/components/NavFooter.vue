...
<template>
	...
	<div class="flex flex-col items-center gap-4 text-center text-[var(--ui-bg)] p-4 text-base-200 bg-gradient-to-r from-primary-600 to-primary-800">
		<div class="flex flex-wrap justify-center font-medium gap-4">
			<NuLink class="hover:underline !text-[var(--ui-bg)]" :to="$localePath('privacy-policy')">
				{{ $t('footer.privacy') }}
			</NuLink>
			<NuLink class="hover:underline !text-[var(--ui-bg)]" :to="$localePath('terms')">
				{{ $t('footer.terms') }}
			</NuLink>
			<NuLink class="hover:underline !text-[var(--ui-bg)]" :to="$localePath('attributions')">
		...
		<div class="text-sm">
			...
			<i18n-t keypath="footer.property" tag="p">
				<strong>The Pokémon Company</strong>
			</i18n-t>
		</div>
	</div>
</template>
...
#stop0 {
	stop-color: var(--color-primary-800);
	stop-opacity: 1;
}
</style>
